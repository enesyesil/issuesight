...
// Imagine you have 3 servers running the same code. If all 3 try to process
// the same GitHub issue at the same time, you'd get duplicate work (or worse,
// corrupted data). A distributed lock prevents this.
//
...
//
// IMPORTANT: Once you have a Lock, YOU are responsible for releasing it!
...
type Lock interface {
	// Release gives up the lock so others can acquire it.
	//
	// IMPORTANT: Only releases if YOU still hold the lock.
	// Returns ErrLockNotHeld if the lock expired or was stolen.
	//
	// ALWAYS use defer to release:
	...
	//
	// WHEN TO USE:
	// If your processing takes longer than expected, extend the lock
	// before it expires. Otherwise, the lock might expire while you're
	// still working, and someone else could grab it!
	//
	// VALIDATION:
	...
	//       processChunk(i)
	//       if i % 10 == 0 {
	//           lock.Extend(ctx, 30*time.Second)  // Reset the timer
	//       }
	//   }
...
type Locker interface {
	...
	//   - ttl must be positive (returns ErrInvalidTTL)
	//   - retries must be >= 0
	//   - retryDelay must be positive if retries > 0
	//
	// EXAMPLE:
	...
	//
	// CAUTION:
	// Be careful with retries - if many workers retry simultaneously,
	// you might create a "thundering herd" problem.
