...
func (_q *UserQuery) QueryTutorials() *TutorialQuery {
...
func (_q *UserQuery) WithTutorials(opts ...func(*TutorialQuery)) *UserQuery {
	query := (&TutorialClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := _q.withTutorials; query != nil {
		if err := _q.loadTutorials(ctx, query, nodes,
			func(n *User) { n.Edges.Tutorials = []*Tutorial{} },
			func(n *User, e *Tutorial) { n.Edges.Tutorials = append(n.Edges.Tutorials, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *UserQuery) loadTutorials(ctx context.Context, query *TutorialQuery, nodes []*User, init func(*User), assign func(*User, *Tutorial)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(tutorial.FieldUserID)
	}
	query.Where(predicate.Tutorial(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.TutorialsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
