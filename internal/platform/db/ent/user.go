...
type User struct {
	...
	// Quota anchor for rate limiting
...
// was not loaded in eager-loading.
func (e UserEdges) TutorialsOrErr() ([]*Tutorial, error) {
	if e.loadedTypes[1] {
...
func (*User) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		switch columns[i] {
		case user.FieldEmail, user.FieldDisplayName, user.FieldAvatarURL:
			values[i] = new(sql.NullString)
		case user.FieldLastRequestedAt, user.FieldCreatedAt:
	...
	for i := range columns {
		...
		case user.FieldLastRequestedAt:
			if value, ok := values[i].(*sql.NullTime); !ok {
				...
				_m.LastRequestedAt = new(time.Time)
				*_m.LastRequestedAt = value.Time
			}
...
// QueryTutorials queries the "tutorials" edge of the User entity.
func (_m *User) QueryTutorials() *TutorialQuery {
	return NewUserClient(_m.config).QueryTutorials(_m)
}
